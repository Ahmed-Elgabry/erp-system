...
//        dd($data);
    ...
    public static function send_twilio_msg($to, $msg, $created_id = 0)
    {
        ...
        try {
            ...
            // Fetch SMS settings for the provided user ID
            $smsSettingsNotifications = SmsSettingNotification::where('Created_by', $created_id)->get();

            if ($smsSettingsNotifications->isNotEmpty()) {

                $rows = [];
                foreach ($smsSettingsNotifications as $row) {
                    $rows[$row->name] = $row->value;
                }

                $smsApiKey = $rows['sms_api_key'];
                $smsSenderName = $rows['sms_sender_name'];

                // Send the notification message using the retrieved SMS settings
